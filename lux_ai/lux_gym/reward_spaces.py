...
class ResearchTimingTracker:
    """
    One-off bonus for unlocking coal/uranium ahead of schedule, shared by the reward
    spaces that use it.

    Unlike a per-step delta this is an EVENT: it pays on the single step a player's
    research first crosses the requirement, and `paid` stops it repeating for the rest
    of the episode. Tiers are checked richest-first and do NOT stack - crossing at
    turn 59 pays 0.2, not 0.3.

    Bonuses are ABSOLUTE, not comparative: if both players research early, both keep
    their bonus. That is why GameResultWithResearchTiming is not zero-sum.
    """
    TIER_VALUES = (0.2, 0.1)
    # Two resources, richest tier each. NB: a genexp here would not see
    # TIER_VALUES - comprehensions get their own scope inside a class body.
    MAX_BONUS = 2. * max(TIER_VALUES)

    def __init__(self, coal_turn: int = 65, coal_fast_turn: int = 60,
                 uranium_turn: int = 165, uranium_fast_turn: int = 160):
        fast, slow = self.TIER_VALUES
        self.tiers = {
            "COAL": ((coal_fast_turn, fast), (coal_turn, slow)),
            "URANIUM": ((uranium_fast_turn, fast), (uranium_turn, slow)),
        }
        self.paid = {key: np.zeros(2, dtype=bool) for key in self.tiers}
        # Turn each requirement was met. NaN until it happens - the trainer drops
        # NaNs before averaging, so a game where uranium is never researched is
        # excluded rather than skewing the mean.
        self.turn = {key: np.full(2, np.nan) for key in self.tiers}
        self._clear_turns_next_update = False

    def update(self, game_state: Game, research_points: np.ndarray) -> np.ndarray:
        """Bonus earned on THIS step, per player. Zero on every step but the crossing."""
        if self._clear_turns_next_update:
            # Deferred from reset(): see the comment there.
            for key in self.tiers:
                self.turn[key][:] = np.nan
            self._clear_turns_next_update = False
        bonus = np.zeros(2, dtype=float)
        for resource, tiers in self.tiers.items():
            requirement = GAME_CONSTANTS["PARAMETERS"]["RESEARCH_REQUIREMENTS"][resource]
            for player in range(2):
                if self.paid[resource][player] or research_points[player] < requirement:
                    continue
                self.paid[resource][player] = True
                self.turn[resource][player] = game_state.turn
                for turn_threshold, value in tiers:
                    if game_state.turn < turn_threshold:
                        bonus[player] += value
                        break
        return bonus
    ...
    def reset(self) -> NoReturn:
        for key in self.tiers:
            self.paid[key][:] = False
        # Do NOT clear `turn` here. reset() runs on the terminal step, and LoggingEnv
        # reads get_info() AFTER that - which is the one step monobeast aggregates
        # over. Clearing now would report NaN for every finished episode. Defer the
        # clear to the first update() of the next episode instead.
        self._clear_turns_next_update = True

    def info(self) -> Dict[str, np.ndarray]:
        return {
            "LOGGING_%s_research_turn" % resource.lower(): turns.astype(np.float32)
            for resource, turns in self.turn.items()
        }
...
class GameResultWithResearchTiming(GameResultReward):
    """
    Win/loss, plus an ABSOLUTE bonus for unlocking coal/uranium early.

    Paid once per game, at the terminal step - not per step - so the reward keeps
    GameResultReward's `only_once=True` shape and the value head keeps the range it
    was trained with. Bonuses accumulate during the episode and are handed over at
    the end, alongside the +1/0/-1 result.

    NOT zero-sum, deliberately. The bonus rewards researching early in absolute
    terms: if both players are fast, both keep it. A zero-sum variant would subtract
    the mean and so reward only being faster than the opponent, which is a different
    objective. The cost is that BaselineLayer uses a Sigmoid value head here rather
    than GameResultReward's Softmax, because a Softmax head structurally forces
    v0 = -v1 and cannot represent both players gaining.
    """
    ...
    def get_reward_spec() -> RewardSpec:
        return RewardSpec(
            reward_min=-1.,
            reward_max=1. + ResearchTimingTracker.MAX_BONUS,
            zero_sum=False,
            only_once=True
        )
    ...
    def __init__(
            ...
            research_timing: float = 1.,
            coal_turn: int = 65,
            coal_fast_turn: int = 60,
            uranium_turn: int = 165,
            uranium_fast_turn: int = 160,
            **kwargs
    ):
        super(GameResultWithResearchTiming, self).__init__(early_stop=early_stop, **kwargs)
        self.research_timing_weight = research_timing
        self.research_timing = ResearchTimingTracker(
            coal_turn=coal_turn, coal_fast_turn=coal_fast_turn,
            uranium_turn=uranium_turn, uranium_fast_turn=uranium_fast_turn)
        self.accumulated_bonus = np.zeros(2, dtype=float)

    def compute_rewards(self, game_state: Game, done: bool) -> Tuple[float, float]:
        # Accrue every step: the crossing can only be detected as it happens, but the
        # payout is deferred so the reward stays terminal-only.
        self.accumulated_bonus += self.research_timing.update(
            game_state, count_research_points(game_state))
        if not done:
            return 0., 0.

        rewards = [int(GameResultReward.compute_player_reward(p)) for p in game_state.players]
        rewards = (rankdata(rewards) - 1.) * 2. - 1.
        rewards = rewards + self.research_timing_weight * self.accumulated_bonus
        self.accumulated_bonus = np.zeros(2, dtype=float)
        self.research_timing.reset()
        return tuple(rewards)

    def get_info(self) -> Dict[str, np.ndarray]:
        return self.research_timing.info()
...
class StatefulMultiReward(FullGameRewardSpace):
    ...
    def __init__(
            ...
            early_stop: bool = False,
            # Turn thresholds for the research-timing bonus. The *_fast_turn tier
            # pays 0.2, the other 0.1; they do not stack.
            coal_turn: int = 65,
            coal_fast_turn: int = 60,
            uranium_turn: int = 165,
            uranium_fast_turn: int = 160,
        ...
        assert coal_fast_turn <= coal_turn, "the 0.2 tier must be the harder one"
        assert uranium_fast_turn <= uranium_turn, "the 0.2 tier must be the harder one"
        ...
        self.weights = {
            ...
            "step": 0.,
            # One-off bonus for unlocking coal/uranium ahead of schedule. Set to
            # 0. to disable; the turn thresholds are the *_turn kwargs above.
            "research_timing": 1.,
        }
    ...
    def compute_rewards(self, game_state: Game, done: bool) -> Tuple[float, float]:
        ...
        reward_items_dict = {
            ...
            "research_timing": self.research_timing.update(game_state, new_research_points),
        }
