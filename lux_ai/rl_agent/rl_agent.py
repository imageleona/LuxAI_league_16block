...
class RLAgent:
    ...
    def preprocess(self, obs, conf) -> NoReturn:
        ...
        if obs["step"] > 0:
            ...
            # NB: the Kaggle entrypoint (main.py) passes an Observation whose
            # `player` is an ATTRIBUTE, not a dict key - obs["player"] raises
            # KeyError there and kills the agent on turn 0. Only the evaluation
            # harness passes a plain dict. Support both.
            self.game_state.id = obs["player"] if "player" in obs else getattr(obs, "player", 0)
