...
def _get_embedding_dim(
        ...
        adaptive_embedding_max_values: Optional[int] = None
) -> int:
    """Return a compact embedding only for configured low-cardinality features.

    A two-value feature needs one dimension, a three- or four-value feature
    needs two. Features above ``adaptive_embedding_max_values`` retain the
    configured width, preserving their original representation capacity.
    """
    if n_values < 2:
        raise ValueError(f"Discrete features must have at least two values, got {n_values}.")
    if max_embedding_dim < 1:
        raise ValueError(f"embedding_dim must be positive, got {max_embedding_dim}.")
    if adaptive_embedding_max_values is None or n_values > adaptive_embedding_max_values:
        return max_embedding_dim
    return min(max_embedding_dim, (n_values - 1).bit_length())
...
class ConvEmbeddingInputLayer(nn.Module):
    def __init__(
            ...
            adaptive_embedding_dims: bool = False,
            adaptive_embedding_max_values: Optional[int] = None,
        ...
        for orig_key, val in obs_space.spaces.items():
            ...
            if key.endswith("_COUNT"):
                ...
                if isinstance(val, gym.spaces.MultiBinary):
                    ...
                    n_values = int(val.nvec.ravel()[0])
                    padding_idx = None
                else:
                    raise NotImplementedError(f"Got gym space: {type(val)}")
                n_players = val.shape[1]
                n_embeddings = n_players * (n_values - 1) + 1
                feature_embedding_dim = (
                    _get_embedding_dim(n_values, embedding_dim, adaptive_embedding_max_values)
                    if adaptive_embedding_dims else embedding_dim
                )
                self.embedding_dims[key] = feature_embedding_dim
                embeddings[key] = nn.Embedding(n_embeddings, feature_embedding_dim, padding_idx=padding_idx)
                if sum_player_embeddings:
                    n_embedding_channels += feature_embedding_dim
                else:
                    n_embedding_channels += feature_embedding_dim * n_players
    ...
    def forward(self, x: Tuple[Dict[str, torch.Tensor], torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns an output of shape (b * 2, out_dim, x, y) where the observation has been duplicated and the
        ...
        for key, op in self.keys_to_op.items():
            ...
            player_indices = torch.arange(in_tensor.shape[2], device=in_tensor.device)
            opponent_indices = torch.flip(player_indices, dims=(0,))
            in_tensor = torch.stack((
                in_tensor,
                in_tensor.index_select(2, opponent_indices),
            ), dim=2)
