...
def create_opponent_model(
    ...
    """
    Builds a frozen league-opponent model: the architecture comes from the
    opponent's own flags, but the obs space is the training env's (with the
    prefix selecting which view of a MultiObs dict the opponent reads).
    """
    return _create_model(model_flags, device, obs_space, obs_space_prefix)
...
def _create_model(
    ...
    conv_embedding_input_layer = ConvEmbeddingInputLayer(
        ...
        # Ported from the team-g fork. getattr with a default, not flags.x, so every
        # config written before these existed still builds the original architecture:
        # adaptive_embedding_dims=False reproduces a uniform embedding_dim exactly.
        adaptive_embedding_dims=getattr(flags, "adaptive_embedding_dims", False),
        adaptive_embedding_max_values=getattr(flags, "adaptive_embedding_max_values", None),
