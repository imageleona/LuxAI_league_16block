...
class DictActor(nn.Module):
    ...
    def forward(
        ...
        compute_actions=False returns an empty actions dict. Selecting actions
        means a multinomial draw over every tile of every action space, which
        is a large share of the forward pass - and the learner uses only
        policy_logits and baseline, so it would throw the result away.
        """
        policy_logits_out = {}
        actions_out = {}
        b, _, h, w = x.shape
