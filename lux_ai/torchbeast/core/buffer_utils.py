...
def create_buffers(
    ...
    if league_conf and league_conf.get("enabled", False):
        # 1. for seats controlled by the learning agent, 0. for frozen league
        # opponents. Lives on dim 1 (the env dim) so it survives split_buffers.
        specs["league_mask"] = dict(size=(t + 1, n, p), dtype=torch.float32)
